...
impl Board {
    ...
    fn check(&self, mv: u32) -> Option<u32> {
        for i in 0..5 {
            if self.squares[5 * i..5 * i + 5].iter().all(|s| s.1)
                || [
                    self.squares[i],
                    self.squares[5 + i],
                    self.squares[10 + i],
                ...
                .all(|s| s.1)
            {
                let sum = self
                    .squares
                    .iter()
                    .fold(0, |acc, s| if !s.1 { acc + s.0 } else { acc });
                let res = mv * sum;
                return Some(res);
            }
        }
...
fn part1(input: &str) -> u32 {
    ...
    let mut boards: Vec<_> = boards
        ...
        .map(|l| Board {
            squares: l
                .split_whitespace()
                .filter_map(|n| Some((n.parse().unwrap(), false)))
                .collect(),
        })
        .collect();
    for m in moves {
